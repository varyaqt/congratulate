...
Текущая версия приложения не работает под Npdejs 18.
Протестировано под Nodejs 16.16.0.
Установить ее можно либо непосредственно с сайта, либо (рекомендуется) с помощью утилиты `nvm`, позволяющей быстро переключаться между версиями Node из командной строки (`nvm install 16.16.0`, `nvm use 16.16.0`).
